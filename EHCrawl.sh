...
list_items=($(echo $list_html | grep -o 'https://exhentai.org/g/[^"]*/'))
...
for gallery_link in "${list_items[@]}"; do
    ...
    if echo $gallery_html | grep -q "Torrent Download ( 0 )" ; then
        ...
        torrents_link="https://exhentai.org/gallerytorrents.php?gid=$id&t=$token"
        ...
        for torrent in "${torrents_list[@]}"; do
            ...
            torrent_link=$(echo $torrent | grep -o 'https://ehtracker.org/get/[^"]*\.torrent?' | sed 's/.$//')
